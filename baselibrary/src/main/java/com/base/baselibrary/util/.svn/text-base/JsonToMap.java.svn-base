...
public class JsonToMap {
	...
	public static <T> List<T> toMapList(String key, String json, Class<T> mClass){
		...
		Map<String,List> classMap= JsonUtils.fromJson(json,Map.class);
		List list2 = classMap.get(key);
		for(int i = 0;i<list2.size();i++){
			String userStr = JsonUtils.toJson(list2.get(i), false);
			T cl = JsonUtils.fromJson(userStr, mClass);
			list.add(cl);
		}
		return list;
	}
	...
	public static <T> List<T> toList(String json, Class<T> mClass){
		List<T> list = new ArrayList<>();
		List personList= JsonUtils.fromJson(json,List.class);
		list.clear();
		for(int i = 0;i<personList.size();i++){
			String userStr = JsonUtils.toJson(personList.get(i), false);
			T cl = JsonUtils.fromJson(userStr, mClass);
			list.add(cl);
		}
		return list;
	...
	public static <T> T toOgject(String json, Class<T> mClass){
		return JsonUtils.fromJson(json, mClass);
	}
